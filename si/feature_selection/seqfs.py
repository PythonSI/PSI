...
class SequentialFeatureSelection:
    ...
    def run(self, x: npt.NDArray[np.floating], y: npt.NDArray[np.floating], covariance: Optional[npt.NDArray[np.floating]] = None) -> npt.NDArray[np.floating]:
    ...
    def fit(
        ...
        if self.criterion is not None:
            ...
            self.active_set, _ = self._stepwise_selection(y, X, self.n_features_to_select, self.direction)
    ...
    def inference(self, z: float) -> Tuple[list, npt.NDArray[np.floating]]:
        ...
        if self.criterion is not None:
            ...
            if self.criterion == "aic":
                itvCriterion = self.interval_AIC(x,
                                        lst_P, len(self.active_set), 
                                        a, b, Sigmatilde, z)
            elif self.criterion == "bic":
                itvCriterion = self.interval_BIC(x,
                                        lst_P, len(self.active_set), 
                                        a, b, Sigmatilde, z)
            elif self.criterion == "adj_r2":
                itvCriterion = self.interval_adjr2(x,
                                        lst_P, len(self.active_set), 
                                        a, b, z)
                
        itv_stepwise = self._interval_stepwisefs(x, len(self.active_set), lst_SELECk,lst_P, a, b, z)
    ...
    def list_residualvec(self, X, Y) -> list:
        ...
        for k in range(s, X.shape[1] + 1):
            ...
            X_Mk = X[:, selec_k].copy()
            lst_Portho.append(I - np.dot(np.dot(X_Mk, np.linalg.inv(np.dot(X_Mk.T, X_Mk))), X_Mk.T))
